...
const securityMiddleware = async (req, res, next) => {
  try {
    ...
    if (decision.isDenied && decision.reason.isBot()) {
      logger.warn(
        `Bot request blocked by security middleware: ${decision.reason}`,
    ...
    if (decision.isDenied && decision.reason.isShield()) {
      logger.warn(
        `Shield request blocked by security middleware: ${decision.reason}`,
        {
          ip: req.ip,
    ...
    if (decision.isDenied && decision.reason.isRateLimit()) {
      logger.warn(
        `Rate limit exceeded by security middleware: ${decision.reason}`,
        { ip: req.ip, userAgent: req.get['User-Agent'], path: req.path }
      );
    ...
    res
      ...
      .json({
        ...
        message: 'Security check failed middleware',
      });
  }
};
